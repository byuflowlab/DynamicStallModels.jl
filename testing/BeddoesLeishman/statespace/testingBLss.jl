...
S = [0.05, 0.05] #Todo: I think this needs to be adjusted for each airfoil, so something matches the static data. - #Todo: this isn't currently used.... werid. 
Tp = 1.7
Tf = 3.0
Tv = 6.0
Tvl = 7.5
...
Cn, Cpn, Cdn, Ccn, Cina, Cinq, u, du = parsesolution_liftonly(sol, p)
...
cycleplot = plot(alfavec[100:end], Cn[100:end], lab="BL", leg=:topleft)
...
ylims!((-1.0,1.5))
display(cycleplot)
# savefig("/Users/adamcardoza/Box/research/FLOW/bladeopt/figures/dynamicstall/beddoesleishmanss/leishman1989fig5/NACA0012_fig5_experimentalslope_081921.png")
