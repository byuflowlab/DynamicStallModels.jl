...
function getintermediatestates(states, U, a, c, dcndalpha, A1, A2, b1, b2)
    ...
    Ncirc_q = dcndalpha_circ*q/2 - X3 -X4
    ...
    return Cfsn, Cvn, Nnoncirc_aq, Ncirc_q, Ncirc_aq, fpp, Nnoncirc_a, Nnoncirc_q, Talpha, M
end
...
function extractintermediatestates(states, Uvec, c, airfoil; a=343.0)
    ...
    for i = 1:n
        Cfsn[i], Cvn[i], Nnc_aq[i], Nc_q[i], Nc_aq[i], fpp[i], Nnc_a[i], Nnc_q[i], Talpha[i], M[i] = getintermediatestates(states[i,:], Uvec[i], a, c, airfoil.dcndalpha, airfoil.A[1], airfoil.A[2], airfoil.b[1], airfoil.b[2])
    end

    return Cfsn, Cvn, Nnc_aq, Nc_q, Nc_aq, Nnc_a, Nnc_q, Talpha, M
end