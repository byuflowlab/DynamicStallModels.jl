#=
I kind of snapped and just copied and pasted the OpenFAST v3.3.0 code over and started translating. Having now done a first pass, I see that this might be a significant amount of work to get this to work. But at least I have it. It's something that I can always come back to and work on. I need to determine what are the states, and everything that needs to be passed into the function. But the nuts and bolts of the function "should" be in place. 
Adam Cardoza 12/9/22
=#
...
function Get_ExpEqn( dt, T, Y_minus1, x, x_minus1 )
    ...
    return Y_minus1*exp(tmp) + (x - x_minus1)*exp(0.5*tmp)
end 
    
    
function updatestates_BADG(oldstates, U, a_s, C_nalpha, Flookup, dt)

    #Unpack previous states

    alpha_minus1, alpha_filt_minus1, Kalpha_f_minus1, q_minus1, q_f_minus1, Kq_f_minus1, Cn_pot_minus1, Df_minus1, Df_c_minus1, Df_m_minus1, fprime_c_minus1, fprime_m_minus1 = oldstates
    ...
    ds = 2.0*U*dt/c     # Eqn 1.5b
       
    # if FirstPass[i,j] #Note: Old states. 
    #    alpha_minus1 = alpha      
    ...
    dynamicFilterCutoffHz = maximum(1.0, U)*filtCutOff/(pi*c)
    ...
    q_cur = Kalpha*c/U   #Kalpha here uses the low-pass filtered alphas (Eqn 1.8 [2])
    ...
    Kalpha_f = q_f_cur*U/c  # Kalpha_f is using the low-pass filtered q (Eqn. 1.8 [4])
    ...
    T_f = T_f0 / sigma1   # Eqn 1.37    
    T_fc = T_f0 / sigma1c   # NOTE: Added equations for time constants of fc (for Cc) and fm (for Cm) with UAMod=2
    # T_fm  = T_f0 / sigma1m[i,j]
      
    Kprime_alpha  = Get_ExpEqn(dt, T_alpha, Kprime_alpha_minus1, Kalpha_f, Kalpha_f_minus1 )    # Eqn 1.18b

    Cn_alpha_nc   = 4.0*T_alpha * ( Kalpha_f - Kprime_alpha ) / M       # Eqn 1.18a
       
    Kprime_q = Get_ExpEqn(dt, T_q, Kprime_q_minus1, Kq_f, Kq_f_minus1)    # Eqn 1.19b 
    ...
    Kprimeprime_q = Get_ExpEqn(dt, (k_mq^2)*T_I , Kprimeprime_q_minus1,  Kq_f, Kq_f_minus1)      # Eqn 1.29 [3]
           
    ### Compute Cm_q_nc 
    # if UAMod == UA_MinnemaPierce
    ...
    Cm_q_nc = -7.0 * (k_mq^2) * T_I * (Kq_f - Kprimeprime_q) / (12.0*M)  
    
    Cc_pot = C_nalpha_circ * alpha_e * alpha #Todo: Used u.alpha.... what is u.alpha versus other values? Should this be the filtered value? They swapped it out for tan(alpha_e+alpha0)
       
    # if ( UAMod == UA_Gonzalez ) then
    #     Cc_pot = C_nalpha_circ * alpha_e * u%alpha  #Added this equation with (u%alpha) instead of tan(alpha_e+alpha0). First, tangent gives problems in idling conditions at anglesof attack of 90 degrees. Second, the angle there is a physical concept according to the original BL model, and u%alpha could be more suitable 
    # else   
    ...
    Dp  = Get_ExpEqn(ds, T_p, Dp_minus1, Cn_pot, Cn_pot_minus1 )     # Eqn 1.35b
    ...
    if (flookup) #Todo: Put in the fseparation function here. 
        # call Get_f_from_Lookup( UAMod, u%Re, u%UserProp, alpha_f, alpha0, C_nalpha_circ, AFInfo, ErrStat2, ErrMsg2, f=fprime)     # Solve Eqn 1.32a for f (=fprime) when alpha isreplaced with alpha_f (see issue when C_nalpha_circ is 0) 
        # call SetErrStat(ErrStat2,ErrMsg2,ErrStat,ErrMsg,RoutineName)
        # if (ErrStat >= AbortErrLev) return
    else   
    ...
    #       Df = 0.0
    #    else
    #       Df = Get_ExpEqn( ds, T_f, Df_minus1, fprime, fprime_minus1)   # Eqn 1.36b
    #    end if
    ...
    fprime_c   = Get_f_c_from_Lookup( UAMod, u%Re, u%UserProp, alpha_f, alpha0, C_nalpha_circ, eta_e, AFInfo, ErrStat2, ErrMsg2) #Todo: 
    ...
    Df_c = Get_ExpEqn( ds, T_fc, Df_c_minus1, fprime_c, fprime_c_minus1)
    Df_m = Get_ExpEqn( ds, T_fm, Df_m_minus1, fprime_m, fprime_m_minus1)  # used in UAMod=UA_Gonzalez only
    ...
    fprimeprime_m   = fprime_m - Df_m
        ...
        #     fprime_c   = Get_f_c_from_Lookup( UAMod, u%Re, u%UserProp, alpha_f, alpha0, C_nalpha_circ, eta_e, AFInfo, ErrStat2, ErrMsg2) # Solve Eqn 1.32b for f when alpha is replaced with alpha_f
            ...
            #      Df_m = 0.0
            #   else
            #      Df_c = Get_ExpEqn( ds, T_fc, Df_c_minus1, fprime_c, fprime_c_minus1(i,j)  )
    ...
    Cn_FS   = Cn_alpha_q_nc + Cn_q_circ + Cn_alpha_q_circ *  ( (1.0 + 2.0*sqrt(fprimeprime) ) / 3.0 )^2
    ...
    C_V   = Cn_alpha_q_circ * ( 1.0 - ((1.0 + 2.0*sqrt(fprimeprime) )/3.0)^2  ) 
    ...
    T_V = T_V0 / sigma3    # Eqn 1.48

    if (tau_V > T_VL)&&(Kalpha_f*dalpha0>0)
        Cn_v = Cn_v_minus1*exp(-2.0*ds/T_V)
    else
        Cn_v = Get_ExpEqn( ds, T_V, Cn_v_minus1, C_V, C_V_minus1 )  # Eqn 1.47
    end
