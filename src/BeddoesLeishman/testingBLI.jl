...
function alpha(t)
    ...
    conv = pi/180   
    degs =  amp*cos(what*2*v*t/c) + shift
    return degs*conv 
end
...
# Tp = 1/0.4125
# Tf = 1/0.0875
S = [2.75, 1.4] #S = [0.05, 0.05] #Note: Increasing S made by "bubble" fatter. 
tspan = [0.0, 1.5]
alpha1 = alphav
...
Cld, tvec, Cpn, Ccn, Cin = BeddoesLeishman(alpha, U, tspan, c, A, b, S, dCndalpha, maxcl, alpha1, Tf, Tp, Tvl; n=1000, a=343.3, f0=NaN)

Clld = zeros(length(Cld))
for i = 1:length(Cld)
    Clld[i] = Cld[i]/cos(alpha(tvec[i]))
end
...
aoavec = collect(0.0:0.05:12).*(pi/180)
clo =  dCndalpha.*(aoavec.-alpha0)  # CL0.(tvec) 
...
cycleplot = plot(alphavec, Cld, lab="dynamic", leg=:topleft)
plot!(alphavec, Cls, lab="static")
plot!(aoavec.*(180/pi), clo, lab="inviscid")
scatter!(expdata[:,1], expdata[:,2], lab="experimental")
# plot!(alphavec, Clld, lab="Rotated") #MEh, that didn't do much. 
xlabel!("angle (degrees)")
ylabel!("Coefficient of Lift")
display(cycleplot)

# savefig("/Users/adamcardoza/Box/research/FLOW/bladeopt/figures/dynamicstall/BLI/clvaoa091621a.png")
