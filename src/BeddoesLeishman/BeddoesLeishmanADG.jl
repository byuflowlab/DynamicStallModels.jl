...
function getloads_BLAG(dsmodel::BeddoesLeishman, states, p, airfoil)
    ...
    Cn, Cc, Cl, Cd, Cm = BLADG_coefficients(dsmodel::BeddoesLeishman, states, U, c, clfit, cdfit, dcndalpha, alpha0, Cd0, Cm0, A1, A2, b1, b2, Tvl, xcp, eta, a)
    return [Cn, Cc, Cl, Cd, Cm]
end


function flookup_gonzalez(airfoil, alpha, dCndalpha_circ)
...
function update_states_ADG(dsmodel::BeddoesLeishman, oldstates, c, a, U, deltat, aoa, dcndalpha, alpha0, A1, A2, b1, b2, Tf0, Tv0, Tp, Tvl, Cn1, afidx)  #Todo: Ryan seems tot think that all of this airfoil information should pass in from the airfoil struct just fine and not affect how derivatives are calculated. 
    ...
    if firstpass_m==1 #TODO: Does this even get used? I'm not storing Ka... so I don't think this gets used... which means I probably have a misnaming on Ka and Kalpha... great -> But I'm getting q right, so I don't know if it matters. 
        Ka_m = 0
    end
    ...
    Kq = (q - q_m)/deltat #Equation 1.8e #Todo: This is mixing unfiltered and filtered q. Beh. This means that I have to store the filtered and unfiltered. 
    if firstpass_m==1
        Kq_m = 0
    end
    ...
    Nnoncirc_q = -Tq*(Kq - Kpq)/M #Equation 1.19a, Noncirculatory component of normal force due to changes in pitching rate. #TODO. The documentation conflicts on whether or not a minus should be included here. -> I'm going to assume that it is positive. And if I'm wrong... I'll change it. It seems to be correct with positive. -> OpenFAST v3.3.0 UnsteadyAero.f90 line 496 has the negative. (No noticeable change.) -> I'm getting the opposite of OpenFAST... so I'm going to take the negative back out. WAIT... I didn't have the negative in the coefficients function down below... So I don't know if it was taking place where I needed it to take place. And I didn't have in my intermediate function to see these unsaved, intermediate results. 
    ...
    # Ncirc_q = dcndalpha_circ*(qf - X3 - X4)/2 #EQ 1.16, Circulatory component of the normal force coefficient response to step change in pitch. #Checked against OpenFAST v3.3.0 #Todo. Where does this get used? -> In the Cn_FS calculation. #TODO: This is different than the calculation in OpenFAST v3.3.0
    ...
    states[18] = fp = flookup_gonzalez(airfoil, alphaf, dcndalpha_circ) #From OpenFAST v3.3.0 #Todo: This function needs to join the others as a multiple dispatch function in the file airfoils.jl. (I don't know how to pass in the dcndalpha_circ... or beta... Maybe a second class of multiple dispatch? )
...
function BLADG_coefficients(dsmodel::BeddoesLeishman, states, U, c, af::Airfoil, a)
    ...
    return BLADG_coefficients(dsmodel, states, U, c, clfit, cdfit, dcndalpha, alpha0, Cd0, Cm0, A1, A2, b1, b2, Tvl, xcp, eta, a)
end

function BLADG_coefficients(dsmodel::BeddoesLeishman, states, U, c, clfit, cdfit, dcndalpha, alpha0, Cd0, Cm0, A1, A2, b1, b2, Tvl, xcp, eta, a)
