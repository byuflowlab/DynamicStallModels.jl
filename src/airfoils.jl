...
function make_simpleairfoil(polar, dsmodel::DSModel)
    ...
    xcp = 0.2
    return Airfoil(dsmodel, polar, cl, cd, cm, cn, cc, dcldalpha, dcldalpha, alpha0, alphasep, alphacut, cutrad, sfun, xcp)
end
...
"""
function make_airfoil(polar, dsmodel::DSModel; xcp=0.2, sfun::Union{SeparationPoint, Function}=ADSP(1, 1), alphacut = [-45, 45].*pi/180, cutrad = 5*pi/180, verbose=false, radians=true) 
...
"""
    update_airfoil(airfoil::Airfoil; dsmodel::DSModel=nothing, polar=nothing, dcldalpha=nothing, dcndalpha=nothing, alpha0=nothing, alphasep=nothing, alphacut=nothing, cutrad=nothing, sfun=nothing, xcp=nothing)
...
"""
function update_airfoil(airfoil::Airfoil; dsmodel::Union{DSModel, Nothing}=nothing, polar=nothing, dcldalpha=nothing, dcndalpha=nothing, alpha0=nothing, alphasep=nothing, alphacut=nothing, cutrad=nothing, sfun=nothing, xcp=nothing, fit=Akima)
    ...
    return Airfoil(newdsmodel, newpolar, newcl, newcd, newcm, newcn, newcc, newslope, newdcndalpha, newalpha0, newalphasep, newalphacut, newcutrad, newsfun, newxcp)
end
