...
@testset "Oye" begin
    ...
    file_2 = "../data/Faber_0015_Oye_Top_Half.csv"

    file_3 = "../data/Faber_0015_Oye_Bottom_Half.csv"

    polar = readdlm(file , ',') #reads in Faber's static lift polar for the NACA_0015 airfoil

    faber_top = readdlm(file_2, ',') #reads in the dynamic lift values from Faber's NACA 0015 (top half)
    Lift_Top_Half = Akima(faber_top[:,1].*pi/180, faber_top[:,2])

    faber_bot = readdlm(file_3, ',') #reads in the dynamic lift values from Faber's NACA 0015 (bottom half)
    Lift_Bottom_Half = Akima(faber_bot[:,1].*pi/180, faber_bot[:,2])

    dsmodel = Oye(Functional(), 1, 2, 4.0)

    af = dsm.make_airfoil(polar, dsmodel, c; sfun=dsm.OSP())
    af = dsm.update_airfoil(af; dcldalpha=2*pi)
    ...
    # nstart = 30
    # plt = plot(answer[1,nstart:end], answer[2,nstart:end])
    # plot!(faber_top[:, 1].*(pi/180), faber_top[:,2])
    # plot!(faber_bot[:, 1].*(pi/180), faber_bot[:,2])
    # display(plt)
    ...
    @testset "Oye Functional Test" begin
        ...
        @testset "State Value" begin
            ...
            @test all((Ref(0) .<= f .<= Ref(1)))
        end
        
        @testset "Fully Separated Lift" begin
            C_fs = dsm.cl_fullysep_faber.(Ref(af), alpha.(sol.t))
            Cl = af.cl.(alpha.(sol.t))

            #test to see if the fully separated lift is always positive and below the static lift; 
            @test all(C_fs<=Cl)
            @test all(Ref(-0.05).<=C_fs)
            ...
            #test to see if the derivative near 32 degrees is pi/6
        ...
        @testset "Dynamic Lift Compared to Faber" begin 
           for i in 30:length(sol.t)-1 #compares the lfit from Faber to our Lift using the Functional Method method
                ...
                relative_error = (answer[2,i]-Faber_Lift)/maximum(polar[:,2]) #relative maximum error
                @test abs(relative_error)<=0.05

                # @test isapprox(answer[2,i], Faber_Lift, atol = 0.07) #Todo. Failing. -> Switch to 0.07 from 0.05 
            end
        end
    end
